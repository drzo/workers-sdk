...
const HelloWorldBindingSchema = z.record(
	z.object({
...
export const HelloWorldOptionsSchema = z.object({
	helloWorld: HelloWorldBindingSchema.optional(),
});
		...
		const services = configs.map<Service>((config) => ({
			...
			worker: {
				...
				bindings: [
					{
						name: "enable_timer",
						json: JSON.stringify(config.enable_timer ?? false),
					},
					{
						name: "name",
						text: JSON.stringify(config.enable_timer ?? false),
					},
					{
						name: "store",
