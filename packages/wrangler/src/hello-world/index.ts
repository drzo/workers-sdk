...
export const helloWorldGetCommand = createCommand({
	metadata: {
		description: "Get a value from the account",
		status: "experimental",
		owner: "Workers: Authoring and Testing",
		hidden: true,
	},
	...
	async handler(args, { config }) {
		...
		if (args.remote) {
			throw new UserError("Not implemented. This is just an example.", {
...
export const helloWorldSetCommand = createCommand({
	metadata: {
		description: "Set a value to the account",
		status: "experimental",
		owner: "Workers: Authoring and Testing",
		hidden: true,
	},
	...
	async handler(args, { config }) {
		...
		await usingLocalHelloWorldBinding(args.persistTo, config, (api) =>
			api.set(args.value)
		);
